...
public class Puzzle15 {
	...
	static class Droid {
		...
		Direction directionToPreviousLocation = null;

		Droid(IntComputer intComputer) {
			this.x = 0;
			this.y = 0;
		...
		Droid(Droid droid) {
			this.x = droid.x;
			this.y = droid.y;
			this.distanceFromStart = droid.distanceFromStart;
			this.intComputer = droid.intComputer.copy();
			this.currentLocation = droid.currentLocation;
		}

		Optional<Droid> performTaskUntil(Predicate<Droid> completionCondition) {
			while(true) {
				if(completionCondition.test(this)) {
				...
				if(possibleDirections.size() == 0) {
					...
					Direction direction = possibleDirections.get(0);
					move(direction);
				} else {
					List<Optional<Droid>> results = new ArrayList<>();
					for (Direction direction : possibleDirections) {
						results.add(branchPointInDirection(direction, completionCondition));
					}
					return results.stream().filter(Optional::isPresent).map(Optional::get).max(Comparator.comparingInt(Droid::getDistance));
				}
			}
		}
		...
		private List<Direction> getPossibleDirections() {
			...
			for(Direction direction : Direction.values()) {
				intComputer.processInput(direction.inputLong);
				Long feedback = intComputer.getOutput().remove();
				if(feedback != 0) {
					intComputer.processInput(direction.reverse.inputLong);
					intComputer.getOutput().remove(); //clean up outputQueue
		...
		void move(Direction direction) {
			...
			direction.move(this);
			distanceFromStart++;
			directionToPreviousLocation = direction.reverse;
			currentLocation = intComputer.getOutput().remove();
		}

		private Optional<Droid> branchPointInDirection(Direction direction, Predicate<Droid> predicate) {
			Droid newDroid = this.copy();
			newDroid.move(direction);
			return newDroid.performTaskUntil(predicate);
		}
		...
		void reset() {
			this.distanceFromStart = 0;
			this.directionToPreviousLocation = null;
		}
	}
	...
	private enum Direction {
		...
		Direction(long inputLong) {
			this.inputLong = inputLong;
		}

		public void move(Droid droid) {
			switch(this) {
			case NORTH: droid.y =  droid.y +1; break;
			case SOUTH: droid.y = droid.y -1; break;
			case WEST: droid.x = droid.x - 1; break;
			case EAST: droid.x = droid.x + 1; break;
			default: throw new RuntimeException("shouldnt happen");
			}
		}
	}
}
